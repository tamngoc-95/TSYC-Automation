...
the system encounters true ambiguity or a recovery condition that cannot be resolved deterministically; or
products are READY_FOR_DRAFT and WooCommerce draft creation requires authorization.
---
...
WooCommerce DRAFT creation for a historical (`FB-HIST-`) candidate that already passed the historical draft-safe readiness gate (section 6.2) -- live-pipeline candidates still stop at the section 6/6.1 human gate.
---
...
6.2 Historical-migration draft-safe exception (explicit shop-owner business authorization)
Adopted 2026-09-04, explicit business authorization from the shop owner, scoped ONLY to historical candidates (candidate_code prefixed `FB-HIST-`, i.e. `is_historical_candidate_code()` in `scripts/create_internal_product.py`). Live-pipeline candidates (`FB-2026-...` and any future non-`FB-HIST-` prefix) are completely unaffected and keep every rule in 6/6.1 unchanged.
For an `FB-HIST-` candidate only, WooCommerce DRAFT creation (`status="draft"`, never `"publish"`, never a price field) is a reversible, pre-authorized migration operation once the historical draft-safe readiness gate passes (`src.domain.rules.readiness_rules.evaluate_historical_draft_safe_readiness`, section 16.1). It does NOT require the per-batch human authorization in 6/6.1, and `scripts/run_batch.py` dispatches it without `--allow-woo-draft` (derived state `READY_FOR_DRAFT_HISTORICAL`, `human_gate=False`).
The historical draft-safe contract requires only:
a meaningful title (verified or extracted)
a known candidate_type (clear, non-conflicted sellable unit)
identity_status is not IDENTITY_CONFLICT (IDENTITY_PENDING is acceptable; full IDENTITY_VERIFIED is not required)
APPROVED customer-facing content
exactly one selected main image: VALIDATED, publish eligible, rights publishable
no active recovery condition
no already-created Woo sync
It explicitly does NOT require: ISBN, author, publisher, weight, page count, dimensions, a second reference, or IDENTITY_VERIFIED. Reference discovery is enrichment, not a mandatory blocker (search order PUBLISHER > AUTHORIZED_SUPPLIER > BOOKSTORE > FAHASA; found references enrich the product, an unfound reference is recorded as a warning and the candidate still proceeds). Every one of these relaxations is a non-blocking warning on the internal product / audit, never a silent invention of the missing fact (section 2.2 still applies in full).
Still hard blockers for a historical candidate, unchanged: a confirmed IDENTITY_CONFLICT, no usable title, unclear/unknown candidate_type, unapproved content, an unusable/missing/multiple main image, an active recovery condition, an existing Woo sync, a genuine sellable-unit or evidence conflict requiring business judgment (section 5.2 continues to apply in full to historical candidates).
This exception never touches: WooCommerce publish (2.4/7 unchanged), selling-price automation (2.5/7 unchanged), deletion or destructive overwrite of verified/approved data (2.7/7 unchanged), or the no-blind-retry rule (2.6/18/19.3 unchanged).
---
7. Hard Deny Rules
...
committing .env
Note: the historical-migration draft-safe exception (section 6.2) is an explicit, narrowly-scoped, still-fully-gated business policy -- applying it is not "disabling" the readiness/identity/image-rights gates listed above. Those gates still run in full for every historical candidate; only their specific pass/fail thresholds differ from the live pipeline, exactly as written in section 6.2/9.4/14.7/16.1. Publish, price mutation, and blind retry remain absolutely denied for historical candidates too.
---
8. Source Priority
...
9.4 Historical draft-safe identity (see section 6.2)
For an `FB-HIST-` candidate only, `create_internal_product.py` may proceed at `identity_status = IDENTITY_PENDING` (never `IDENTITY_CONFLICT`), with zero or a `POSSIBLE_MATCH`/`MANUAL_REVIEW` reference used only as enrichment, not as the mandatory MATCH reference section 13 otherwise requires. `IDENTITY_VERIFIED` is preferred and used whenever the evidence already supports it -- this is not a relaxation of what counts as verified, only of what is required before a historical draft can be created. A confirmed `IDENTITY_CONFLICT` still stops the candidate, exactly as section 9.2 requires for every candidate.
---
...
14.7 Historical-migration image-rights auto-authorization (see section 6.2)
Adopted 2026-09-04, explicit shop-owner business authorization, scoped to `FB-HIST-` candidates only:
An image sourced from the shop's own Facebook export is automatically classified `STORE_OWNED` -- no separate per-image human rights approval.
An image downloaded from an already-approved reference `source_type` is automatically classified at the mapped rights status: `PUBLISHER` -> `PUBLISHER_APPROVED`; `AUTHORIZED_SUPPLIER` / `BOOKSTORE` / `FAHASA` -> `SUPPLIER_APPROVED` (`src.domain.rules.image_rules.APPROVED_REFERENCE_SOURCE_RIGHTS`).
An image from any other/unconfigured source is NOT automatically approved and still requires human rights review (`RIGHTS_UNKNOWN` / `REVIEW_REQUIRED`), exactly as section 14.3/14.4 already require for everything outside an established policy basis.
This auto-authorization never substitutes for the separate, still-mandatory candidate-relevance/ownership check -- an auto-authorized-rights image must still be confirmed to actually depict this candidate before it can be selected as the main image (section 14.5/14.6 unchanged).
---
15. Content Workflow
...
16.1 Historical draft-safe readiness (see section 6.2)
For an `FB-HIST-` candidate, `READY_FOR_DRAFT` may be reached under the relaxed contract in section 6.2 (`evaluate_historical_draft_safe_readiness`) instead of the full readiness list above. `scripts/check_draft_readiness.py` and `scripts/pipeline_state.py` select the historical gate automatically from the candidate_code prefix -- no separate script or duplicated business logic. The live-pipeline readiness list above is completely unchanged for every non-historical candidate.
---
17. WooCommerce Draft Creation
Woo creation occurs only after explicit bounded human authorization.
Exception: for a historical (`FB-HIST-`) candidate that reached `READY_FOR_DRAFT` under the section 6.2/16.1 draft-safe policy, this authorization is the explicit shop-owner business authorization itself -- no further per-batch confirmation is requested. Every other requirement on this page (draft-only status, no price fields, exact SKU, pre-POST duplicate/recovery/readiness re-check) applies identically to historical and live-pipeline candidates.
Always create:
status = "draft"
Never include:
regular_price
...
Continue until READY_FOR_DRAFT.
Ask user once for Woo draft authorization.
Create authorized drafts serially.
Reconcile.
Audit.
Stop.

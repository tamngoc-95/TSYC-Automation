...
CONTENT_REVIEW_REQUIRED
9.5 Content Style Source of Truth
Before planning, generating, revising, translating, reviewing, validating, or approving any customer-facing VI/EN/DE product content, Cowork and Claude Code must read:
docs/content_style/TSYC_CONTENT_GUIDE.md
and the relevant approved examples under:
docs/content_style/examples/
Rules:
The content guide and approved examples control tone, structure, wording, localization style, and presentation.
Verified product data, approved references, candidate-specific evidence, and persisted provenance remain the factual source of truth.
Never copy product-specific facts from an approved example into another product unless independently verified for that candidate.
This includes:
ISBN
author
translator
...
shipping wording
translated titles
Vietnamese remains the canonical semantic reference for multilingual content unless an explicit project rule states otherwise.
English and German must be natural localizations of the verified Vietnamese meaning, not literal machine-style translations and not independently invented descriptions.
...
If style guidance conflicts with CLAUDE.md or CLAUDE_AUTOMATION.md safety/business rules, those safety/business rules win.
Uncertainty, missing-data notes, validator reasons, review instructions, and internal workflow messages must remain in operator/review/log fields and must never be inserted into customer-facing storefront descriptions.
Do not invent or pad content merely to satisfy a target paragraph count, description length, or example structure.
Only explicitly approved examples belong in the canonical examples set. Unapproved drafts must not be treated as style references.
A content-writing execution prompt generated by Cowork must explicitly instruct Claude Code to read the content guide and relevant approved examples before performing the content operation.
WooCommerce Multilingual Rule
...
When the selected or likely next action involves content drafting, revision, translation, review, validation, approval, or multilingual consistency, also read:
docs/content_style/TSYC_CONTENT_GUIDE.md
relevant approved examples under docs/content_style/examples/
Do not treat style examples as factual evidence for another product.
...
Claude Code Execution Prompt Contract
...
no-price rule
no-publish rule
For any content-related execution, the prompt must also require:
read docs/content_style/TSYC_CONTENT_GUIDE.md before writing or reviewing content
read the relevant approved examples under docs/content_style/examples/
use verified candidate evidence as the factual source of truth
use the content guide/examples only for tone, structure, wording, localization style, and presentation
preserve Vietnamese as the canonical semantic reference for EN/DE localization unless an explicit project rule states otherwise
never invent or copy unverified product facts from style examples
...
Non-Historical / Live Workflow Protection
